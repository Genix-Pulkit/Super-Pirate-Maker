...
class Editor:
	...
	def create_grid(self):
		...
		for tile_Pos, tile in self.canvas_data.items():
			...
			if tile.coin:
				layers['coins'][(x, y)] = tile.coin
			if tile.enemy:
				layers['enemies'][(x + TILE_SIZE // 2, y + TILE_SIZE // 2)] = tile.enemy
			if tile.objects:
	...
	def object_drag(self, event):
		...
		if event.type == pygame.MOUSEBUTTONUP and self.object_drag_active:
			for sprite in self.canvas_objects:
				sprite.end_drag(origin = self.origin)
				self.object_drag_active = False
		...
		if not self.menu.rect.collidepoint(mouse_pos()):
			if selected_object:
				...
				points = [
					...
					((rect.right - size, rect.bottom), rect.bottomright, (rect.right, rect.bottom - size))
				]
				for point in points:
					pygame.draw.lines(
				...
				if type_dict[self.selection_index] == 'tile':
					...
					rect = surf.get_rect(center = self.origin + mouse_pos())
	...
	def create_clouds(self, event):
		if event.type == self.cloud_timer:
			...
			pos = [WINDOW_WIDTH + randint(20,50), randint(0, WINDOW_HEIGHT)]
...
class CanvasTile:
	...
	def get_water(self):
		return 'water' if self.water_on_top else 'top'
